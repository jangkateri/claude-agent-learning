...
async def main(prompt: str, options_str: str = "{}"):
    ...
    if result_message is not None:
        ...
        print(f'  Duration:      {result_message.duration_ms}ms (API: {result_message.duration_api_ms}ms)')
        print(f'  Total Cost:    ${result_message.total_cost_usd}' if result_message.total_cost_usd is not None else '  Total Cost:    N/A')
        print(f'  Is Error:      {result_message.is_error}')
        print(f'  Stop Reason:   {result_message.stop_reason}' if result_message.stop_reason is not None else '  Stop Reason:   N/A')
...
if __name__ == "__main__":
    prompt = " ".join(sys.argv[1:]) if len(sys.argv) > 1 else "Who are you?"
